...
class Qr_code
{
	...
	var $quality = true;
	var $size = 1024;
	var $mainUrl = 'https://rabbani.id/';
	var $sandboxUrl = 'https://sandbox.rabbani.id/';
	...
	public function defaultParams($payload = []) {
		$data = (isset($payload['mode']) && $payload['mode']=='production') ? $this->mainUrl : $this->sandboxUrl;
	...
	public function generate($params = array()) {
		...
		$isData = $defaultParams['data']; 
		if(isset($params['data']) && $params['data'] && !empty($params['data']) && !is_null($params['data'])){
			$isData = $params['data'];
		}
		...
		$saveName = $defaultParams['save'];
		if (isset($params['savename']) && $params['savename'] && !empty($params['savename']) && !is_null($params['savename'])){
			$saveName = $params['savename'];
		}

		$padding = $defaultParams['padding'];
		if (isset($params['padding']) && is_numeric($params['padding'])){
			$padding = min(max((int)$params['padding'], 1), 5);
		}
		QRcode::png($isData, $saveName, $level, $size, $padding);
	}
}
