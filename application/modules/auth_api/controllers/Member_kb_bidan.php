...
class Member_kb_bidan extends REST_Controller {
    ...
    public function list_get () {
        ...
        $result = $handler->getMemberKbBidans($filters);
        if ($result->hasErrors()) {
    ...
    public function index_post () {
        ...
        $handler = new MemberKbManager($this->MainModel, $auth->data);
        $result = $handler->createMemberKbBidan($payload);
        if ($result->hasErrors()) {
            $this->response($result->format(), $result->getStatusCode());
        }
    ...
    public function update_post ($id) {
        ...
        $payload = $this->input->post();
        $handler = new MemberKbManager($this->MainModel, $auth->data);
        $result = $handler->updateMemberKbBidans($payload, $filters);
        if ($result->hasErrors()) {
            $this->response($result->format(), $result->getStatusCode());
        }
    ...
    public function delete_post ($id) {
        ...
        $payload = $this->input->post();
        $handler = new MemberKbManager($this->MainModel, $auth->data);
        $result = $handler->deleteMemberKbBidans($filters);
        if ($result->hasErrors()) {
            $this->response($result->format(), $result->getStatusCode());
        }
    ...
    public function import_post () {
        ...
        foreach ($data as $d) {
            ...
            $existsData = $handler->getMemberKbBidan($args)->data;
            if (!empty($existsData)) {
                $updateData[] = $existsData;
                $action = $handler->updateMemberKbBidans($d, ['id' => $existsData->id]);
            } else {
                $newData[] = $d;
                $action = $handler->createMemberKbBidan($d);
            }
        }
